...
class ScheduleEraserPolicyTest extends TestCase
{
    ...
    public function view_any()
    {
        ...
        $this->assertTrue($user->can('viewAny', ScheduleEraser::class));

        $user = self::createPermissionedUser('view blocks', false);
        $this->assertFalse($user->can('viewAny', ScheduleEraser::class));
    }
    ...
    {
        $eraser = ScheduleEraser::factory()->make([
            'creator_id' => $user,
        ]);

        $this->assertEquals($expected, $user->can($permission, $eraser));
    }
